...
namespace Cobilas.Unity.Management.InputManager
{
    public static class InputCapsuleUtility
    {
        ...
        public static void ChangeTriggerInInputCapsule(
          InputCapsule capsule,
          KeyPressType pressType,
          bool SecondaryTrigger) {
            InputCapsuleTrigger[] inputCapsuleTriggers = GetKeyInput.GetInputCapsuleTriggers();
            if (ArrayManipulation.EmpytArray(inputCapsuleTriggers))
                return;
            if (!capsule.IsFixedInput) {
                capsule.SetIsChange(true);
                inputCapsuleTriggers[inputCapsuleTriggers.Length - 1].SetKeyPressType(pressType);
                if (SecondaryTrigger) SetSecondaryTrigger(capsule, inputCapsuleTriggers);
                else SetTriggerFirst(capsule, inputCapsuleTriggers);
            }
        }

        internal static void SetTriggerFirst(
          InputCapsule capsule,
          params InputCapsuleTrigger[] triggers) {
            if (capsule.IsFixedInput)
                return;
            capsule.SetTriggerFirst(triggers);
        }

        internal static void SetSecondaryTrigger(
          InputCapsule capsule,
          params InputCapsuleTrigger[] triggers) {
            if (capsule.IsFixedInput)
                return;
